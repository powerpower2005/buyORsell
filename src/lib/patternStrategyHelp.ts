...
export const PATTERN_STRATEGY_HELP: Record<PatternStrategyId, HelpContent> = {
  breakout_entry: {
    ...
    howToFind:
      "차트 패턴이 ‘완성(confirmed)’으로 잡힌 봉 = 목선·저항·지지 종가 돌파 봉입니다. 이 전략은 그 완성 봉에 바로 진입 마커를 올립니다. 다우·갠의 ‘추세 전환 첫 확인(a)’·엘리어트 돌파 감각에 가깝습니다.",
    ...BREAK,
  ...
  triple_confirm: {
    ...
    worksWith: "모든 클래식 차트 패턴. 페이크 돌파 경고와 함께 보면 실패 케이스를 걸러내기 쉽습니다.",
    pros:
      "말씀하신 ‘종가·거래량·리테스트’ 원칙을 한 전략으로 고정해 불필요한 손실을 줄입니다. 가짜 돌파에 가장 강합니다.",
    cons:
  ...
  fake_breakout: {
    ...
    howToFind:
      "패턴이 종가 돌파로 완성된 뒤 약 15봉 안에, 돌파했던 넥라인·레벨을 다시 종가로 관통하면 실패·페이크입니다. 마커 방향은 재관통 쪽(실패 후 가격이 간 방향)입니다.",
    ...BREAK,
    higher:
      "숏 패턴 돌파 실패 후 레벨 위로 종가 복귀 → 롱 쪽 경고(숏 무효·되돌림).",
    lower:
      "롱 패턴 돌파 실패 후 레벨 아래로 종가 재이탈 → 숏 쪽 경고(롱 무효).",
    worksWith: "돌파·리테스트·삼중 확인. 페이크가 뜨면 기존 패턴 진입을 무효로 보세요.",
    pros: "패턴 실패를 규칙으로 표시해 ‘모양만 보고 버티기’를 막습니다.",
    cons:
      "짧은 휩쏘도 페이크로 잡힐 수 있고, 실패 후 재돌파가 나오면 다시 판단해야 합니다.",
    tip: "불필요한 손실을 줄이려면 페이크 경고 시 손절·청산을 규칙화하세요.",
  },
};
