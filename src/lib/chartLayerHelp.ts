...
export const CHART_LAYER_HELP = {
  allStrategies: {
    ...
    tip: "옆 %는 이 종목 전 구간 추적 성공률입니다(목표·손절 ±1.5%, 표본 3건↑). 지표만 다시 꺼도 전략 마커는 유지됩니다.",
  },
  strategyConfluence: {
    title: "전략 겹침",
    summary:
  ...
  volume: {
    title: "거래량",
    summary:
      "거래량은 가격을 실제로 움직이는 힘입니다. 모든 캔들은 거래량의 결과이며, 관심·세력의 흔적을 보여 줍니다.",
    howToFind:
      "패널 막대와 3/7/15/30 평균선을 비교하세요. 가격이 크게 올라도 거래량이 미미하면 신뢰↓, 조금만 올라도 평균 대비 몇 배면 큰 자금 유입으로 신뢰↑. 상승 추세 중 조정에서 거래량이 줄면 쉬어가기, 하락 중 거래량이 줄면 매도세 약화·반등 준비 후보입니다. 약세 다이버전스(가격↑·거래량↓)는 신규 매수 자제·익절 준비, 강세 다이버전스(가격↓·거래량↑)는 반등 후보—다만 가격↓+거래량↑만으로 무조건 매수하지 마세요(의도된 하락일 수 있음).",
    higher:
      "추세 방향으로 거래량이 뒷받침되면 추세 신뢰도가 높아지고, 같은 방향 포지션 승률이 나아지는 편입니다.",
    lower:
      "거래량이 약한 구간은 거짓 신호·금방 무너질 움직임일 수 있어 진입을 줄입니다.",
    worksWith:
      "히트맵·볼륨 파이트·VSA 전략, EMA60·파라볼릭 SAR, MACD·일목·추세선 돌파, MFI. 돌파·패턴 완성봉에 거래량이 붙으면 신뢰↑.",
    tip: "거래량이 많이 터진 가격대는 이후 지지·저항이 되기 쉽습니다. 가격↓+거래량↑+다른 지표 상승 다이버전스가 겹치면 강한 매수 후보로 봅니다.",
  },
  volumeStrategies: {
    title: "거래량·VWAP·OBV 전략 마커",
    summary:
      "히트맵·파이트·VSA, VWAP 눌림목·밴드·스위칭, OBV 다이버전스·OBV+켈트너·패스트 OBV 추력과 복합 지표(VWAP자금흐름·켈트너+CCI·OBV+ST)를 표시합니다.",
    howToFind:
      "사이드바 거래량 → 전략에서 개별로 켜세요. OBV 계열은 OBV 패널(+시그널)·켈트너 오버레이를, VWAP 계열은 VWAP을 켜 두면 확인이 쉽습니다. 복합 전략은 보조 지표 쪽에도 같이 나열됩니다.",
    worksWith:
      "거래량 패널·OBV·켈트너·VWAP, EMA(60). 거래량만으로는 매수/매도 구분이 안 되니 OBV로 힘의 방향을 보세요.",
  ...
  comboStrategies: {
    ...
    tip: "도움말의 「어디에 쓰면 좋은지」를 보고 추세장/횡보/장중에 맞게 골라 쓰세요.",
  },
  swing: {
    title: "스윙 구조",
    summary:
  ...
  macdStrategies: {
    ...
    tip: "옆 %는 이 종목 전 구간 추적 성공률입니다. 0선 위/아래 크로스와 Hist를 함께 보면 휩쏘를 줄이는 데 도움이 됩니다.",
  },
  stochStrategies: {
    title: "스토캐스틱 전략 마커",
    summary:
