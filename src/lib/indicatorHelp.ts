...
export const INDICATOR_HELP: Record<string, HelpContent> = {
  ...
  macd: {
    ...
    summary:
      "단기 EMA(보통 12)에서 장기 EMA(보통 26)를 뺀 값으로 추세 방향·강도를 봅니다. 시그널(보통 9)과 Hist(오실레이터)로 전환을 더 빨리 봅니다. 다우이론이 ‘추세 지속 기간·전환 시점’을 안 알려 주는 약점을 오실레이터로 보완하려는 대표 지표입니다.",
    howToFind:
    ...
    pros:
      "이평 교차보다 Hist·다이버전스로 모멘텀 약화·전환 후보를 조금 일찍 볼 수 있습니다. 추세 필터(0선)와 타점(시그널 교차)을 한 패널에서 같이 봅니다.",
    cons:
      "본질은 이동평균 파생이라 후행성이 남습니다. 다이버전스·히스토그램 해석이 주관적이고, 횡보장 가짜 교차·‘얼마나 갈지’ 미제공이 한계입니다.",
    tip: "MACD는 후행성이 있어 Hist·다이버전스·사이드바 MACD 전략으로 시점을 보완합니다.",
  },
  rsi: {
    title: "RSI (상대강도지수) · 슈퍼 RSI",
    summary:
      "최근 상승·하락 폭의 상대 강도로 0~100 모멘텀을 봅니다. 패널에는 기본 RSI(보라)와 함께 슈퍼 RSI식 가중선·유동 과매수/과매도·중심선이 그려집니다. 다우식 추세 확인만으로는 부족한 ‘과열·과매도·다이버전스’를 보완하는 오실레이터입니다.",
    howToFind:
    ...
    pros:
      "추세 지속만 보는 다우/이평의 약점(과열 구간·전환 후보)을 숫자로 보완합니다. 다이버전스로 스윙 고·저점과 모멘텀 어긋남을 볼 수 있습니다.",
    cons:
      "같은 RSI를 보고도 매수/매도 해석이 갈리는 주관성이 큽니다. 고정 70/30은 추세장에서 실패가 잦고, ‘선행’처럼 보이는 다이버전스도 확정은 후행·주관적입니다.",
